...
resource "aws_iam_role" "researcher_lambda_role" {
  name = "wealthix-researcher-lambda-role"
...
resource "aws_iam_role_policy" "researcher_lambda_bedrock_access" {
  name = "wealthix-researcher-lambda-bedrock-policy"
...
resource "aws_lambda_function" "researcher" {
  ...
  function_name = "wealthix-researcher"
  package_type  = "Image"
  ...
  environment {
    variables = {
      ...
      WEALTHIX_API_ENDPOINT = var.wealthix_api_endpoint
      WEALTHIX_API_KEY      = var.wealthix_api_key
...
resource "aws_iam_role" "eventbridge_role" {
  ...
  name  = "wealthix-eventbridge-scheduler-role"
...
resource "aws_lambda_function" "scheduler_lambda" {
  count         = local.scheduler_active ? 1 : 0
  function_name = "wealthix-researcher-scheduler"
  ...
  environment {
    variables = {
      RESEARCHER_URL = trimsuffix(aws_lambda_function_url.researcher[0].function_url, "/")
    }
  }
...
resource "aws_iam_role" "lambda_scheduler_role" {
  count = local.scheduler_active ? 1 : 0
  name  = "wealthix-scheduler-lambda-role"
...
resource "aws_scheduler_schedule" "research_schedule" {
  count = local.scheduler_active ? 1 : 0
  name  = "wealthix-research-schedule"
