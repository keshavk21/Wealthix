"""
Tools for the Wealthix Researcher agent
"""
import os
...
WEALTHIX_API_ENDPOINT = os.getenv("WEALTHIX_API_ENDPOINT")
WEALTHIX_API_KEY = os.getenv("WEALTHIX_API_KEY")
...
def _ingest(document: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with httpx.Client() as client:
        response = client.post(
            WEALTHIX_API_ENDPOINT,
            json=document,
            headers={"x-api-key": WEALTHIX_API_KEY},
            timeout=30.0
        )
...
def ingest_financial_document(topic: str, analysis: str) -> Dict[str, Any]:
    """
    Ingest a financial document into the Wealthix knowledge base.
    ...
    """
    if not WEALTHIX_API_ENDPOINT or not WEALTHIX_API_KEY:
        return {
            "success": False,
            "error": "Wealthix API not configured. Running in local mode."
        }
