"""
Wealthix Researcher Service - Investment Advice Agent
"""
...
from app.context import get_agent_instructions, DEFAULT_RESEARCH_PROMPT
from app.mcp_servers import create_playwright_mcp_server
from app.tools import ingest_financial_document

# Load environment
load_dotenv(override=True)

app = FastAPI(title="Wealthix Researcher Service")
...
async def run_research_agent(topic: str = None) -> str:
    ...
    with trace("Researcher"):
        async with create_playwright_mcp_server(timeout_seconds=120) as playwright_mcp:
            ...
            agent = Agent(
                name="Wealthix Investment Researcher",
...
async def health():
    ...
    return {
        "service": "Wealthix Researcher",
        "status": "healthy",
        "wealthix_api_configured": bool(os.getenv("WEALTHIX_API_ENDPOINT") and os.getenv("WEALTHIX_API_KEY")),
        "timestamp": datetime.now(UTC).isoformat(),
