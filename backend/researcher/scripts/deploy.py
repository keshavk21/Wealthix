...
def write_tfvars(terraform_dir: Path, image_uri: str | None = None):
    ...
    tfvars_data = {
        ...
        "wealthix_api_key": os.environ.get("WEALTHIX_API_KEY", ""),
        "bedrock_region": os.environ.get("BEDROCK_REGION", "ap-south-1"),
        "researcher_model": os.environ.get("RESEARCHER_MODEL", "bedrock/us.amazon.nova-pro-v1:0"),
        "scheduler_enabled": str(os.environ.get("SCHEDULER_ENABLED", "false")).lower() == "true",
    }
    
    if image_uri:
        tfvars_data["researcher_image_uri"] = image_uri
        
    override_path.write_text(json.dumps(tfvars_data, indent=2) + "\n")
...
def wait_for_lambda_active(region: str, function_name: str):
    ...
    for _ in range(60):
        ...
        if status == "Successful" and state == "Active":
            print("[SUCCESS] Lambda is active.")
            return
        if status == "Failed":
            print("[ERROR] Lambda update failed. Check the AWS Console or CloudWatch logs.")
            sys.exit(1)

        print(".", end="", flush=True)
        time.sleep(5)

    print("\n[WARNING] Lambda update is taking longer than expected.")


def main():
    print("Wealthix Researcher Service - Lambda Deployment")
    print("==========================================")
    ...
    terraform_dir = repo_root / "terraform" / "researcher"
    backend_dir = repo_root / "backend" / "researcher"

    print("\nWriting configuration from .env to terraform...")
    write_tfvars(terraform_dir)
    ...
    # Extract region from the ECR URL to ensure we authenticate to the correct region
    ecr_region = ecr_url.split(".")[3]
    
    # Login to ECR
    print(f"\nLogging in to ECR in region {ecr_region}...")
    password = run_command(
        ["aws", "ecr", "get-login-password", "--region", ecr_region], capture_output=True
    )
    ...
    local_image = f"wealthix-researcher:{image_tag}"
    ...
    write_tfvars(terraform_dir, remote_image)
